...
async function testChromaCategoryScraper() {
  const scraper = new ChromaCategoryScraper();
  
  try {
    console.log('🚀 Starting Chroma Category Scraper Test...\n');
    ...
    if (categoryData.products && categoryData.products.length > 0) {
      ...
      if (sampleProduct.additionalInfo) {
        console.log(`   ℹ️  Additional Info:`, sampleProduct.additionalInfo);
      }
    }
    ...
    console.log(`📁 Data saved to: scraped_data/chroma-category-*.json`);
...
if (require.main === module) {
  testChromaCategoryScraper();
}

module.exports = testChromaCategoryScraper;