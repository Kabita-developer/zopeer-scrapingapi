...
class NykaaCategoryScraper {
  ...
  async initialize() {
    try {
      console.log('Nykaa Category scraper initialized successfully');
      return true;
    } catch (error) {
      console.error('Error initializing Nykaa Category scraper:', error);
      throw error;
    }
  }
  ...
  async scrapeCategoryPage(url, usePuppeteer = true) {
    try {
      console.log(`Starting to scrape Nykaa category page: ${url}`);
      
      // Validate URL
      if (!url || !url.includes('nykaa.com')) {
        throw new Error('Invalid Nykaa URL provided');
      }
      ...
      console.error('Error scraping Nykaa category page:', error);
      throw error;
    }
  }
  ...
  async fetchWithPuppeteer(url) {
    ...
    try {
      ...
      try {
        await page.waitForSelector('.product-item, .product-card, .product-listing', { 
          timeout: 15000 
        });
  ...
  extractCategoryName($, url = '') {
    try {
      ...
      if (pageTitle && pageTitle !== 'Nykaa') {
        // Remove common suffixes like "| Nykaa"
        const cleanTitle = pageTitle.replace(/\s*\|\s*Nykaa.*$/i, '').trim();
  ...
  extractProducts($) {
    try {
      ...
      // Extract products from various selectors (Nykaa specific)
      const productSelectors = [
        '.css-1rd7vky', // Main product container
        '.product-item',
        '.product-card',
        '.product-listing',
        '.product-tile',
        '[data-testid="product-item"]',
        '.css-1gb49vx', // Alternative Nykaa selector
        '.css-1gc4x7i'  // Another Nykaa selector
      ];
  ...
  extractProductName($product) {
    try {
      const nameSelectors = [
        '.css-1gb49vx', // Nykaa specific product name
        '.css-1gc4x7i', // Nykaa specific product title
        '.product-name',
        '.product-title',
        '.product-name-link',
        'h3',
        ...
        'a[href*="/p/"]' // Link containing product ID
      ];
    ...
    try {
      ...
      if (productName) {
        ...
        const brands = [
          ...
          'Charlotte Tilbury', 'Juicy Chemistry', 'Everteen', 'Wellbeing Nutrition',
          'Indulgeo Essentials', 'Optimum Nutrition', 'VILVAH', 'Soulflower',
          'GUBB', 'Newsha', 'e.l.f. Cosmetics', 'Lotus Herbals', 'Swiss Image'
        ];
      ...
      // Try to extract from brand element
  ...
  extractPricing($product) {
    try {
      ...
      const priceSelectors = [
        '.css-1jczs19', // Nykaa specific selling price
        '.css-17x6nxn', // Nykaa specific price
        '.price',
        '.selling-price',
        '.offer-price',
        '.current-price',
      ...
      const discountSelectors = [
        '.discount',
        '.off-percentage',
        '.css-17x6nxn span' // Nykaa specific
      ];
      ...
      for (const selector of discountSelectors) {
        ...
        if (element.length > 0) {
          ...
          if (discountText && (discountText.includes('%') || discountText.includes('OFF'))) {
  ...
  extractProductImage($product) {
    try {
      const imageSelectors = [
        ...
        '.product-img img',
        '.css-1rd7vky img', // Nykaa specific
        'img[src*="nykaa"]',
        'img'
      ];
  ...
  extractRatingData($product) {
    try {
      ...
      const ratingSelectors = [
        '.css-m6n3ou', // Nykaa specific rating
        '.rating',
        '.stars',
        '.product-rating',
        '[data-testid="rating"]',
      ...
      const reviewSelectors = [
        '.css-1hvvm95', // Nykaa specific review count
        '.reviews',
        '.review-count',
        '.product-reviews',
  ...
  async saveToJson(categoryData, url) {
    try {
      ...
      const filename = `nykaa-category-${timestamp}.json`;
  ...
  async close() {
    try {
      console.log('Nykaa Category scraper closed successfully');
    } catch (error) {
      console.error('Error closing Nykaa Category scraper:', error);
    }
  }
}

module.exports = NykaaCategoryScraper;
