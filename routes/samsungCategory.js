...
const SamsungCategoryScraper = require('../utilities/samsungCategoryScraper');

// Initialize scraper instance
let samsungCategoryScraper = null;

// Initialize scraper
const initializeScraper = async () => {
  if (!samsungCategoryScraper) {
    samsungCategoryScraper = new SamsungCategoryScraper();
    await samsungCategoryScraper.initialize();
  }
  return samsungCategoryScraper;
};
...
router.get('/health', (req, res) => {
  res.json({
    status: 'OK',
    service: 'Samsung Category Scraper',
    timestamp: new Date().toISOString(),
    version: '1.0.0'
  });
});

// Scrape Samsung category page
  ...
  try {
    ...
    if (!url) {
      return res.status(400).json({
        ...
        message: 'Please provide a valid Samsung category URL'
      });
    }
    ...
    if (!url.includes('samsung.com')) {
      return res.status(400).json({
        ...
        message: 'Please provide a valid Samsung category URL (must contain samsung.com)'
      });
    }
...
// Scrape Samsung category page (GET method)
  ...
  try {
    ...
    if (!url) {
      return res.status(400).json({
        success: false,
        error: 'URL is required',
        message: 'Please provide a valid Samsung category URL as query parameter'
      });
    }
...
router.post('/test', async (req, res) => {
  try {
    // Sample Samsung category URL for testing
    const sampleUrl = 'https://www.samsung.com/in/offer/';
...
router.get('/history', async (req, res) => {
  try {
    ...
    try {
      ...
      const samsungFiles = files.filter(file => file.startsWith('samsung-category-') && file.endsWith('.json'));
      
      const history = [];
      
      for (const file of samsungFiles) {
        try {
...
router.get('/result/:filename', async (req, res) => {
  try {
    ...
    // Validate filename
    if (!filename || !filename.startsWith('samsung-category-') || !filename.endsWith('.json')) {
      return res.status(400).json({
        success: false,
        error: 'Invalid filename',
        message: 'Filename must be a valid Samsung category scraping result file'
      });
    }
...
router.use((error, req, res, next) => {
  console.error('Samsung Category route error:', error);
