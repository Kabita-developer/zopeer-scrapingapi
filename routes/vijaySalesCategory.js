...
let vijaySalesCategoryScraper = null;

// Initialize scraper
const initializeScraper = async () => {
  if (!vijaySalesCategoryScraper) {
    vijaySalesCategoryScraper = new VijaySalesCategoryScraper();
    await vijaySalesCategoryScraper.initialize();
  }
  return vijaySalesCategoryScraper;
};
...
router.get('/health', (req, res) => {
  res.json({
    status: 'OK',
    service: 'Vijay Sales Category Scraper',
    timestamp: new Date().toISOString(),
    version: '1.0.0'
  });
});

// Scrape Vijay Sales category page
  ...
  try {
    ...
    if (!url) {
      return res.status(400).json({
        ...
        message: 'Please provide a valid Vijay Sales category URL'
      });
    }

    // Validate URL format
    if (!url.includes('vijaysales.com') || !url.includes('/c/')) {
      return res.status(400).json({
        success: false,
        error: 'Invalid URL format',
        message: 'Please provide a valid Vijay Sales category URL (must contain vijaysales.com and /c/)'
      });
    }
...
// Scrape Vijay Sales category page (GET method)
  ...
  try {
    ...
    if (!url) {
      return res.status(400).json({
        success: false,
        error: 'URL is required',
        message: 'Please provide a valid Vijay Sales category URL as query parameter'
      });
    }
...
router.post('/test', async (req, res) => {
  try {
    // Sample Vijay Sales category URL for testing
    const sampleUrl = 'https://www.vijaysales.com/c/home-appliances?PRICE=499.00to47561.00&SORT=Popular';
...
router.get('/history', async (req, res) => {
  try {
    ...
    try {
      ...
      const vijaySalesFiles = files.filter(file => file.startsWith('vijay-sales-category-') && file.endsWith('.json'));
      
      const history = [];
      
      for (const file of vijaySalesFiles) {
        try {
...
router.get('/result/:filename', async (req, res) => {
  try {
    ...
    // Validate filename
    if (!filename || !filename.startsWith('vijay-sales-category-') || !filename.endsWith('.json')) {
      return res.status(400).json({
        success: false,
        error: 'Invalid filename',
        message: 'Filename must be a valid Vijay Sales category scraping result file'
      });
    }
...
router.use((error, req, res, next) => {
  console.error('Vijay Sales Category route error:', error);
