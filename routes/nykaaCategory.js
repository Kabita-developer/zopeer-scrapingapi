...
const NykaaCategoryScraper = require('../utilities/nykaaCategoryScraper');
const fs = require('fs').promises;
const path = require('path');

// Initialize scraper instance
let nykaaCategoryScraper = null;

// Initialize scraper
const initializeScraper = async () => {
  if (!nykaaCategoryScraper) {
    nykaaCategoryScraper = new NykaaCategoryScraper();
    await nykaaCategoryScraper.initialize();
  }
  return nykaaCategoryScraper;
};
...
router.get('/health', (req, res) => {
  res.json({
    status: 'OK',
    service: 'Nykaa Category Scraper',
    timestamp: new Date().toISOString(),
    version: '1.0.0'
  });
});

// Scrape Nykaa category page
  ...
  try {
    ...
    if (!url) {
      return res.status(400).json({
        ...
        message: 'Please provide a valid Nykaa category URL'
      });
    }
    ...
    if (!url.includes('nykaa.com')) {
      return res.status(400).json({
        ...
        message: 'Please provide a valid Nykaa category URL (must contain nykaa.com)'
      });
    }
...
// Scrape Nykaa category page (GET method)
  ...
  try {
    ...
    if (!url) {
      return res.status(400).json({
        success: false,
        error: 'URL is required',
        message: 'Please provide a valid Nykaa category URL as query parameter'
      });
    }
...
router.post('/test', async (req, res) => {
  try {
    // Sample Nykaa category URL for testing
    const sampleUrl = 'https://www.nykaa.com/clearance-sale/c/861?intcmp=oz%2Cclearance-native%2C1%2Cclearance-sale&redirectpath=slug&transaction_id=d724dfffd5842b962854a6de55d1ecfe';
...
router.get('/history', async (req, res) => {
  try {
    ...
    try {
      ...
      const nykaaFiles = files.filter(file => file.startsWith('nykaa-category-') && file.endsWith('.json'));
      
      const history = [];
      
      for (const file of nykaaFiles) {
        try {
...
router.get('/result/:filename', async (req, res) => {
  try {
    ...
    // Validate filename
    if (!filename || !filename.startsWith('nykaa-category-') || !filename.endsWith('.json')) {
      return res.status(400).json({
        success: false,
        error: 'Invalid filename',
        message: 'Filename must be a valid Nykaa category scraping result file'
      });
    }
...
router.use((error, req, res, next) => {
  console.error('Nykaa Category route error:', error);
