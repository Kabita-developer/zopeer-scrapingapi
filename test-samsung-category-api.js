...
async function testSamsungCategoryAPI() {
  try {
    console.log('🚀 Testing Samsung Category API...\n');
    
    const baseUrl = 'http://localhost:3000';
    const testUrl = 'https://www.samsung.com/in/offer/';
    
    // Test POST endpoint
    console.log('📤 Testing POST /api/samsung-category/scrape-category...');
    const postResponse = await axios.post(`${baseUrl}/api/samsung-category/scrape-category`, {
      url: testUrl,
    ...
    // Test GET endpoint
    console.log('\n📥 Testing GET /api/samsung-category/scrape-category...');
    const getResponse = await axios.get(`${baseUrl}/api/samsung-category/scrape-category`, {
      params: {
        url: testUrl,
        usePuppeteer: false
      }
    ...
    const healthResponse = await axios.get(`${baseUrl}/api/samsung-category/health`);
    ...
    const historyResponse = await axios.get(`${baseUrl}/api/samsung-category/history`);
...
if (require.main === module) {
  testSamsungCategoryAPI();
}

module.exports = testSamsungCategoryAPI;
