...
async function testChromaCategoryAPI() {
  try {
    console.log('🚀 Testing Chroma Category API...\n');
    
    const baseUrl = 'http://localhost:3000';
    const testUrl = 'https://www.croma.com/home-appliances/c/5?q=%3Adiscount-desc%3AZAStatusFlag%3Atrue%3AskuStockFlag%3Atrue%3AskuStockFlag%3Atrue';
    
    // Test POST endpoint
    console.log('📤 Testing POST /api/chroma-category/scrape-category...');
    const postResponse = await axios.post(`${baseUrl}/api/chroma-category/scrape-category`, {
      url: testUrl,
    ...
    // Test GET endpoint
    console.log('\n📥 Testing GET /api/chroma-category/scrape-category...');
    const getResponse = await axios.get(`${baseUrl}/api/chroma-category/scrape-category`, {
      params: {
        url: testUrl,
        usePuppeteer: false
      }
    ...
    const healthResponse = await axios.get(`${baseUrl}/api/chroma-category/health`);
    ...
    const historyResponse = await axios.get(`${baseUrl}/api/chroma-category/history`);
...
if (require.main === module) {
  testChromaCategoryAPI();
}

module.exports = testChromaCategoryAPI;
